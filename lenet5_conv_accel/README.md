# LeNet-inspired CNN — CONVOLUTION-ACCELERATED variant (Vitis HLS)

Same LeNet-inspired network as the baseline, but with **only the two convolution
layers accelerated**. Everything else (padding, pooling, flatten, both
fully-connected layers) is unchanged from the sequential baseline.

**What's accelerated** — `hls/src/core/convolution2DRelu_1.cpp` and
`convolution2DRelu_2.cpp` now use, per output pixel:
- a **fixed-point** datapath (`ap_fixed<20,8>` data, `ap_fixed<40,16>` accumulate)
  during synthesis — falls back to `float` in C-simulation;
- inputs/weights buffered in on-chip BRAM;
- the output-pixel loop **pipelined (II=1)** with the 5×5 window **fully
  unrolled** and the products summed through a **balanced adder tree**
  (25 taps for conv1, 75 for conv2).

This is the technique from the standalone conv5x5 accelerator, applied in place.
The function signatures (float in/out) are unchanged, so the rest of the network
is untouched.

**Verification**
- **C-simulation (float path): 100 % on the 100-image MNIST set** — the
  accelerated compute is functionally correct.
- **Fixed-point accuracy is NOT tested by C-sim** (C-sim runs the float branch).
  Confirm it with Vitis **C/RTL co-simulation** or an on-board run before you
  trust the numbers; if accuracy drops, widen `ap_fixed` (e.g. more fractional
  bits) in the two conv files.
- **Resource fit:** fixed-point conv is far cheaper than the float-unrolled
  version that overflowed the KV260, and the FC layers stay sequential, so this
  should fit — but only C-Synthesis + implementation will confirm DSP/LUT.

This is the "5×5-accelerated LeNet" of your three-stage set (baseline →
conv-accelerated → fully accelerated). The three components (`lenet5_hls`,
`lenet5_system`, `lenet5_host`) and everything below are identical to the
baseline workspace.

---

Full LeNet-inspired CNN for MNIST in C++ for High-Level Synthesis.
